program main
  ...
  if (I%solver == "Newton-Raphson") then  ! NR

    print*, ('NEWTON RAPHSON SOLVER')
    call NR_solver(I,Sop)


    ! Plots:
    allocate( plot_vec( I%ntNR )  ) ! vector used to plot.
    nelm = I%Nx ! Element chose to plot.
    plot_vec = reshape( NR%d_Ep(1,1,nelm,:) , (/I%ntNR/) )  ! Plastic deformation
    call plot(plot_vec)


  else ! LATIN-PGD

    print*, ('LATIN-PGD SOLVER')
